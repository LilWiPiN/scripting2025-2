...
namespace MoreMountains.CorgiEngine
{
    ...
    public class LevelManager : MMSingleton<LevelManager>, MMEventListener<CorgiEngineEvent>
    {
        ...
        protected virtual void GenerateColliderBounds()
        {
            ...
            if (BoundsMode == BoundsModes.TwoD)
            {
                BoundsCollider2D = this.gameObject.GetComponent<Collider2D>();

                // Si ya es un PolygonCollider2D, no lo modificamos ni lo movemos
                if (BoundsCollider2D is PolygonCollider2D)
                {
                    return; // salimos y dejamos tu polígono tal cual
                }

                // Caso contrario: crea el collider por defecto
                if (BoundsCollider2D != null)
                {
                    DestroyImmediate(BoundsCollider2D);
                }
